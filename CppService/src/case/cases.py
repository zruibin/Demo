...
def check(name):
    ...
    if not os.path.exists(factoryFile):
        log(str(factoryFile) + " was not exist.")
        return False
    return True
...
class ${name} : public BaseCaseAdapter
{
public:
    virtual ~${name}() = default;
};
...
void ${name}::SetAdapter(const std::shared_ptr<BaseCaseAdapter> adapter) {
    if (adapter == nullptr) {
        return;
    }
...
class ${name} : public BaseCaseBuilder
{
public:
    ${name}() = default;
    virtual ~${name}() = default;
...
public:
    std::shared_ptr<BaseCase> BuildCase() override {
        std::shared_ptr<BaseCaseAdapter> adapter = std::make_shared<${adapterName}>();
