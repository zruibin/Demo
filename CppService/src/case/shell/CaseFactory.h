...
public:
    explicit CaseFactory() {
        builderMap_ = std::make_shared<std::unordered_map<std::string, std::shared_ptr<BaseCaseBuilder>>>();
    };
    
public:
    void ConstructBuilders() override;
    
    std::shared_ptr<BaseCaseBuilder> GetBuilder(const std::string& name) override {
        if (name.length() == 0) {
            return nullptr;
        }
    ...
    void UpdateBuilder(std::shared_ptr<BaseCaseBuilder> builder) override {
        ...
        const std::string& name = builder->GetCaseName();
        auto it = builderMap_->find(name);
        if (it != builderMap_->end()) {
            it->second = builder;
        } else {
...
private:
    using BuilderMap = std::shared_ptr<std::unordered_map<std::string, std::shared_ptr<BaseCaseBuilder>>>;
